...
export function PasswordResetForm({ token }: { token: string }) {
  ...
  return <form className="auth-form" onSubmit={submit}><label>New password<input name="password" type="password" autoComplete="new-password" minLength={14} maxLength={128} required /></label><p className="field-help">Use 14–128 characters — a phrase of a few ordinary words works well. Spaces and symbols are allowed, and none are required. Passwords that appear in a public breach, or that contain your own name or email address, are refused.</p><label>Confirm new password<input name="confirmation" type="password" autoComplete="new-password" minLength={14} maxLength={128} required /></label>{error && <p className="form-error" role="alert">{error}</p>}<button className="primary-button full-button" type="submit" disabled={busy}><KeyRound size={17} /> {busy ? "Changing password…" : "Change password"}</button></form>;
}
