...
import type { StaffPasskeySummary } from "@/modules/access/passkeys";
...
export function StaffPasskeyManager({
  ...
  async function add() {
    ...
    setNotice("");
    try {
      const { options } = await postPasskeyJson("/api/auth/passkeys/registration/options");
  ...
  async function remove(passkey: StaffPasskeySummary) {
