/**
 * Pure Square configuration rules, deliberately free of the `server-only`
 * guard so operational tooling can ask the same questions the payment path
 * asks. Import `square-config` from anything that runs inside the app; the
 * guard lives there.
 */
