...
export const DEFAULT_MESSAGE_TEMPLATE_NAMES: Readonly<Record<MessageTemplateKey, string>> = {
  ...
  WAITLIST_REMOVED: "Waitlist removal confirmation",
  REGISTRATION_CANCELLED: "Registration cancellation",
  REGISTRATION_REACTIVATED: "Registration reactivation",
...
> = {
  ...
  WAITLIST_REMOVED:
    "Sent when a registration leaves the waitlist without being promoted.",
  REGISTRATION_CANCELLED:
    "Confirms cancellation while preserving accurate payment and refund wording.",
  REGISTRATION_REACTIVATED:
    "Confirms successful reactivation with the restored status, current balance, attendee summary, and preserved registration history.",
  REGISTRATION_CONTACT_UPDATED:
...
> = {
  ...
  REGISTRATION_REACTIVATED: "Registration reactivated: {{event_name}} ({{confirmation_code}})",
...
export const DEFAULT_MESSAGE_TEMPLATE_BODIES: Readonly<Record<MessageTemplateKey, string>> = {
  ...
  WAITLIST_REMOVED: [
    "# Removed from the waitlist for {{event_name}}",
    "",
    "Hello {{recipient_name}},",
    "",
  ...
  REGISTRATION_REACTIVATED: [
    "# Registration reactivated for {{event_name}}",
    "",
    "Hello {{recipient_name}},",
    "",
...
export const MESSAGE_TEMPLATE_TOKEN_OPTIONS: readonly {
  ...
  {
    key: "waitlist_removal_reason",
    label: "Waitlist removal reason",
    description: "The safe operator-provided reason for removing the registration from the waitlist.",
  },
  {
    key: "contact_email",
