...
import { honorOfferingInputSchema } from "@/modules/honors/schemas";
...
async function postHandler(request: Request, context: { params: Promise<{ eventId: string }> }) {
  ...
  try {
    ...
    const input = honorOfferingInputSchema.parse(await request.json());
    return Response.json(await createHonorOffering(eventId, input, access.user.id), { status: 201 });
  } catch (error) {
    return honorApiError(error, "Adding an honor offering");
  }
}
