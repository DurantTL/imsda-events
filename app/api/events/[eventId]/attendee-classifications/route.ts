...
import { attendeeClassificationInputSchema } from "@/modules/attendee-types/domain";
import { AttendeeConfigurationError, createAttendeeClassification, listAttendeeConfiguration } from "@/modules/attendee-types/repository";
...
async function getHandler(_request: Request, context: { params: Promise<{ eventId: string }> }) {
  try {
    ...
    return Response.json({ classifications: (await listAttendeeConfiguration(eventId)).classifications });
...
async function postHandler(request: Request, context: { params: Promise<{ eventId: string }> }) {
  ...
  try {
    ...
    const input = attendeeClassificationInputSchema.parse(await request.json());
    return Response.json({ classification: await createAttendeeClassification(eventId, access.user.id, input) }, { status: 201 });
  } catch (error) { return apiError(error); }
}
