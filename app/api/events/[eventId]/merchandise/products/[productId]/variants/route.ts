...
import { createVariant } from "@/modules/merchandise/catalog-repository";
import { withRequestContext } from "@/lib/request-context";

type Context = { params: Promise<{ eventId: string; productId: string }> };
export const POST = withRequestContext(async (request: Request, context: Context) => {
  ...
  try {
    ...
    return Response.json({ variant: await createVariant(eventId, productId, await request.json(), access.user.id) }, { status: 201 });
  } catch (error) {
