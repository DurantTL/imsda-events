...
import { publishPolicyVersion } from "@/modules/consent/repository";
...
async function postHandler(request: Request, context: { params: Promise<{ eventId: string; policyId: string }> }) {
  ...
  try {
    ...
    const version = await publishPolicyVersion(eventId, policyId, access.user.id, await request.json());
