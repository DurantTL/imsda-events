...
import { TagConfigurationError, removeRegistrationTag } from "@/modules/tags/repository";
...
async function deleteHandler(request: Request, context: { params: Promise<{ eventId: string; registrationId: string; tagId: string }> }) {
  const originError = rejectCrossOriginRequest(request);
  if (originError) return originError;
  try {
    const { eventId, registrationId, tagId } = await context.params;
    const access = await requirePermission(await getCurrentSession(), eventId, "MANAGE_REGISTRATION", findActiveMembership);
    const assignment = await removeRegistrationTag(eventId, registrationId, tagId, access.user.id);
